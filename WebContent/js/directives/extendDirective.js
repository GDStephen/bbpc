...
(function (root, factory) {
    ...
    'use strict';
    var module = angular.module('extendDirective', []);

    module.directive('imageOnLoad', ['$parse', function ($parse) {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
                element.bind('load', function () {
                    ...
                    var invoker = $parse(attrs.imageOnLoad);
                    invoker(scope, {param: argString});
                });
            }
        };
    }]);

    module.directive('imageOnError', [function () {
        return {
            restrict: 'A',
            link: function (scope, element, attrs) {
    ...
    module.directive('bbcContextMenu', ['$parse', '$q', function ($parse, $q) {
         ...
         * @param $scope
         * @param event 事件对象
         * @param options 右键菜单数组
         * @param model
         * @param level
         * @param customClass 右键的css类
         *
         */
        function renderContextMenu($scope, event, options, model, level, customClass) {
            ...
            //构建具体的菜单，循环右键菜单数组，如果迭代值为null则构建一个分隔符
            angular.forEach(options, function (item) {
                var needAdd = true;
                if (item !== null && typeof item[2] === 'function') {
                    var isHide = item[2].call($scope, $scope, event, model).hide;
                    if (isHide) needAdd = false;
                }
                if (needAdd) {
                    var $li = $('<li>');
                    if (item === null) {
...
//                    custom.initialize($li, item);
            ...
            $contextMenu.bind("mousedown", function (e) {
         ...
         * @param level
         */
        function processItem($scope, event, model, item, $ul, $li, $promises, $q, $, level) {
            ...
            var isEnabled = function () {
                if (typeof item.enabled !== "undefined") {
                    ...
                    var isDisable = item[2].call($scope, $scope, event, model, text).isDisable;
                    if (isDisable === true)
                        return false;
                    return true;
                } else {
         ...
         */
        function processTextItem($scope, item, text, event, model, $promises, nestedMenu, $) {
            "use strict";
            ...
            if (typeof item[2] === 'function') {
                var newText = item[2].call($scope, $scope, event, model).newText;
        ...
        function registerEnabledEvents($scope, enabled, item, $ul, $li, nestedMenu, model, text, event, $, level) {
        ...
        function handlePromises($ul, level, event, $promises) {
            /// <summary>
        ...
        return {
            ...
            link: function (scope, element, attrs) {
                ...
                element.bind(openMenuEvent, function (event) {
                    ...
                    scope.$apply(function () {
                        var options = scope.$eval(attrs.bbcContextMenu);
                        ...
                        if (options instanceof Array) {
                            ...
                            throw '"' + attrs.bbcContextMenu + '" 需要为一个数组';
                        }
                    });
                });
            }
    ...
    module.directive('bbcRepeatEnd',['$parse',function ($parse) {
        return function (scope, element, attrs) {
            if (scope.$last) {
                var invoker = $parse(attrs.bbcRepeatEnd);
                invoker(scope);
            }
        };
    }]);
